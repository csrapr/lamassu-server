...
const blacklist = require('../blacklistQueries')
const db = require('../db')
...
function post(machineTx, pi) {
  return db.tx(cashInAtomic.atomic(machineTx, pi)).then(r => {
    ...
    return Promise.all([
      ...
      .then(([{ config }, blacklistItems]) => {
        const rejectAddressReuseActive = configManager.getCompliance(config)
          .rejectAddressReuse

        if (_.some(it => it.created_by_operator === true)(blacklistItems)) {
          blacklisted = true
        } else if (
          _.some(it => it.created_by_operator === false)(blacklistItems) &&
  ...
  return pi
    ...
    .then(sendRec => {
      settingsLoader.loadLatest().then(it => {
        const rejectAddressReuseActive = configManager.getCompliance(it.config)
          .rejectAddressReuse
        if (rejectAddressReuseActive) {
          blacklist
            .addToUsedAddresses(r.tx.toAddress, r.tx.cryptoCode)
